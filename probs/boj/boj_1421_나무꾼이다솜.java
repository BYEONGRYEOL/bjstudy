...
public class boj_1421_나무꾼이다솜 {
	...
	static void solve() throws Exception {
		...
		for (int i = 1; i <= 10000; i++) {
			maxValue = Math.max(maxValue, calculateValue(trees, i, value, cost));
		}
		sb.append(maxValue);
	}

	static long calculateValue(int[] trees, int unitLen, int value, int cost) {
		int sell = 0;
		int cut = 0;
		for (int tree : trees) {
			int nowSell = tree / unitLen;
			int nowCut = (int)Math.ceil((double)tree / unitLen) - 1;
			if(nowSell * value * unitLen > nowCut * cost){
				sell += nowSell;
				cut += nowCut;
			}
		}
		return (long)sell * unitLen * value - (long)cut * cost;
	}
