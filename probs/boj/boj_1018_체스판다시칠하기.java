...
public class boj_1018_체스판다시칠하기 {
	...
	static void solve() throws Exception {
		char[][] chessBoard1 = new char[8][8];
		char[][] chessBoard2 = new char[8][8];
		for(int i = 0; i < 8; i++){
			if(i%2 == 0) {
				chessBoard1[i] = "WBWBWBWB".toCharArray();
				chessBoard2[i] = "BWBWBWBW".toCharArray();
			} else {
				chessBoard1[i] = "BWBWBWBW".toCharArray();
				chessBoard2[i] = "WBWBWBWB".toCharArray();
			}
		}
		...
		for(int i = 0; i <= n - 8; i++){
			for(int j = 0; j <= m - 8; j++){
				...
				for(int k = 0; k < 8; k++){
					for(int l = 0; l < 8; l++){
						if(map[i + k][j + l] != chessBoard1[k][l]) count1++;
						if(map[i + k][j + l] != chessBoard2[k][l]) count2++;
					}
				}
