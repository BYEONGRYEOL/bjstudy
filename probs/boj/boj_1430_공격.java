...
public class boj_1430_공격 {
	...
	static void solve() throws Exception {
		...
		int[][] tower = scan.nextIntMatrix(n,2);
		Arrays.sort(tower, Comparator.comparingDouble(a -> distance(a, target)));
		Map<Integer, List<int[]>> depths = new HashMap<>();
		int[] depth = new int[n];
		...
		while(true){
			for (int i = 0; i < n; i++) {
				...
				List<int[]> depthList = depths.get(curDepth-1);
				if(depth[i] == 0){
					for (int[] spot : depthList) {
						// System.out.println("distance(tower[i], spot) : " + distance(tower[i], spot));
						if (depth[i] == 0 && distance(tower[i], spot) <= range) {
							...
							depths.get(curDepth).add(tower[i]);
							depth[i] = curDepth;
						}
					}
				}
		...
		for (Map.Entry<Integer, List<int[]>> entry : depths.entrySet()){

			if(entry.getKey() > 0){
				// System.out.printf("depth : %d towerCount : %d\n", entry.getKey(), entry.getValue().size());
				damage += entry.getValue().size() * (energy / Math.pow(2,entry.getKey() - 1));
