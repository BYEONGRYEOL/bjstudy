...
public class boj_12970_AB {
	...
	static void solve() throws Exception {
		int n = scan.nextInt();
		TARGET = scan.nextInt();
		String[][][] dp = new String[n+1][n+2][TARGET+1];
		//dp[i][j][k] = i 전체길이, j A의 개수, k 현재 점수
		dp[1][0][0] = "B";
		dp[1][1][0] = "A";		
		for(int i = 2; i<=n;i++){
			for(int j = 0; j<=i;j++){
				for(int k = 0; k<=TARGET;k++){
					if(dp[i-1][j][k] != null){
						// A를 추가
						dp[i][j+1][k] = dp[i-1][j][k] + "A";
						if(k+j <= TARGET){
							dp[i][j][k+j] = dp[i-1][j][k] + "B";
						if(k+j == TARGET){
							sb.append(dp[i][j][k+j] + "A".repeat(n - dp[i][j][k+j].length())).append("\n");
							return;
						}
						}
					}
