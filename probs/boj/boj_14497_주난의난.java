...
public class boj_14497_주난의난 {
	...
	static void solve() throws Exception {
		...
		int count = 0;
		while(true){
			int waveResult = waveMove(map, sX, sY, -100);
			count++;
			// printArray(map);
			if(waveResult == 0){
				break;
	...
	static int waveMove(String[] map, int sX, int sY, int notDir){
		for(int i = 0; i < 4; i++){
			if(i + notDir == 1 || i + notDir == 5){
				continue;
			}
			int waveX = sX + dx[i];
			int waveY = sY + dy[i];
			if(waveX < 0 || waveX >= n || waveY < 0 || waveY >= m){
				continue;
			}
			// 파동이 친구를 만나면 친구&파동 제거
			else if(map[waveX].charAt(waveY) == '1'){
				map[waveX] = map[waveX].substring(0, waveY) + '.' + map[waveX].substring(waveY + 1);
				// sb.append(waveX).append(' ').append(waveY).append(' ').append(i).append("친구와 충돌\n");
				continue;
			}
			// 파동이 길을 지나면 확장
			else if(map[waveX].charAt(waveY) == '0')
			{
				// sb.append(waveX).append(' ').append(waveY).append(' ').append(i).append("확장\n");
				int result = waveMove(map, waveX, waveY, i);
				if(result == 0){
					return 0;
				}

			}
			else if(map[waveX].charAt(waveY) == '#'){
				return 0;
			}
		}
		// for문을 다 돈경우 일반적
	...
	static void printArray(String[] map) {
		for (String s : map) {
			sb.append(s).append("\n");
		}
		sb.append("\n");
