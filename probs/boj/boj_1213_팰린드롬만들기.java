...
public class boj_1213_팰린드롬만들기 {
	...
	static void solve() throws Exception {
		...
		int oddCount = 0;
		for (int i = 0; i < 26; i++) {
			oddCount += characters[i] % 2;
		}
		if (oddCount > 1) {
			sb.append("I'm Sorry Hansoo").append('\n');
			return;
		}
		...
		for (int i = 0; i < 26; i++) {
			...
			forward.append(String.valueOf((char)('A' + i)).repeat(characters[i] / 2));
			backward.append(String.valueOf((char)('A' + i)).repeat(characters[i] / 2));
		}
		String answer = "";
		if (centerFlag) {
			answer = forward + String.valueOf(center) + backward.reverse();
		} else {
			answer = forward + backward.reverse().toString();
		}
		sb.append(answer);
	}

	static long boxCount(int[] lengths, double len) {
		return (long)(lengths[0] / len) * (long)(lengths[1] / len) * (long)(lengths[2] / len);
