...
public class boj_17387_선분교차2 {
	static void solve() throws Exception {
		// ccw
		// l1(p1,p2) 를 기준으로 l2(p3,p4) 에 대하여 ccw 계산
		// ccw < 0 : cw
		// ccw > 0 : ccw
		// ccw = 0 : 일직선
		// 두 선분기준 각각 ccw의 값 4개가 모두 0이면 -> 일직선
		// 평행하는 두 선분이 겹치는지 보려면 완전한 포함관계도 고려해야함
		// 선분1ccw 2값 곱 <=0 && 선분2ccw 2값 곱 <= 0 이라면 교점존재
		Point2D p1 = scan.nextIntPoint2D();
		Point2D p2 = scan.nextIntPoint2D();
		Point2D p3 = scan.nextIntPoint2D();
		Point2D p4 = scan.nextIntPoint2D();
		double l1ccw1 = crossProduct(p1, p2, p3);
		double l1ccw2 = crossProduct(p1, p2, p4);
		double l2ccw1 = crossProduct(p3, p4, p1);
		double l2ccw2 = crossProduct(p3, p4, p2);

		if (l1ccw1 == 0 && l1ccw2 == 0 && l2ccw1 == 0 && l2ccw2 == 0) {
			LineSegment2D line1 = new LineSegment2D(p1, p2);
			LineSegment2D line2 = new LineSegment2D(p3, p4);
			boolean isOnLine1 = line1.isOnLineSegment(p3);
			boolean isOnLine2 = line1.isOnLineSegment(p4);
			boolean isOnLine3 = line2.isOnLineSegment(p1);
			boolean isOnLine4 = line2.isOnLineSegment(p2);
			if (isOnLine1 || isOnLine2 || isOnLine3 || isOnLine4) {
				sb.append(1);
			} else
				sb.append(0);
		} else if (l1ccw1 * l1ccw2 <= 0 && l2ccw1 * l2ccw2 <= 0) {
			sb.append(1);
		} else
			sb.append(0);
	}
