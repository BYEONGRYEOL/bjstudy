...
public class boj_1063_킹 {
	...
	static void solve() throws Exception {
		...
		int[] king = new int[2];
		king[0] = '8' - inputs[0].charAt(1);
		king[1] = inputs[0].charAt(0) - 'A';

		int[] stone = new int[2];
		stone[0] = '8' - inputs[1].charAt(1);
		stone[1] = inputs[1].charAt(0) - 'A';
		int n = Integer.parseInt(inputs[2]);
		for (int i = 0; i < n; i++) {
			int[] next =  move(scan.nextLine());
			int nextKingX = king[0] + next[0];
			int nextKingY = king[1] + next[1];
			if(nextKingX < 0 || nextKingX >7 || nextKingY < 0 || nextKingY >7) {
				// 왕탈출
				continue;
			}
			if(nextKingX == stone[0] && nextKingY == stone[1]) {
				// 돌겹침
				int nextStoneX = nextKingX + next[0];
				int nextStoneY = nextKingY + next[1];
				if(nextStoneX < 0 || nextStoneX >7 || nextStoneY < 0 || nextStoneY >7) {
					// 왕탈출
					continue;
				}
				stone[0] = nextStoneX;
		...
		sb.append((char)('A' + king[1])).append(8-king[0]).append('\n');
		sb.append((char)('A' + stone[1])).append(8-stone[0]).append('\n');
	}

	static int[] move(String command) {

		int[] move = new int[] {0, 0};
		move[1] += command.contains("R") ? 1 : 0;
		move[1] -= command.contains("L") ? 1 : 0;
		move[0] += command.contains("B") ? 1 : 0;
		move[0] -= command.contains("T") ? 1 : 0;
		return move;
	}

	static void flip(int[][] board, int[] idx) {
		int n = idx[0];
		int m = idx[1];
		for (int i = 0; i <= n; i++)
			for (int j = 0; j <= m; j++)
	...
	static int[] findBackCoin(int[][] board) {
	...
	private static int aTo0(char c) {
	...
	public static void printStream(Object d) {
