...
public class boj_17612_쇼핑몰 {
    ...
    static void solve() throws Exception {
        int n = scan.nextInt();
        int k = scan.nextInt();
        Custormers[] custormers = new Custormers[n];
        for (int i = 0; i < n; i++) {
            custormers[i] = new Custormers();
            custormers[i].num = scan.nextInt();
            custormers[i].w = scan.nextInt();
            custormers[i].serverNum = i+1;
            custormers[i].endTime = custormers.
        }
        PriorityQueue<Custormers> nextEvent = new PriorityQueue<>(
                (o1, o2) -> o1.endTime == o2.endTime ? -Integer.compare(o1.serverNum, o2.serverNum)
                        : Integer.compare(o1.endTime, o2.endTime));
        
    }

    static class Custormers {
        int num;
        int w;
        int serverNum;
        int endTime;
