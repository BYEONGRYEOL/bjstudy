...
public class boj_9328_열쇠 {
	...
	static void solve() throws Exception {
		// 열쇠 -> 문 위상정렬과 유사한 알고리즘
		// 탐색 시 문에의해 가로막혀있는 셀이 있다면 문->셀의 순서를 가짐,
		// 해당 셀이 방문가능해졌다면, 즉시 방문한것으로 간주하고, 열쇠를 득할 경우 문 해체처리

		int iter = scan.nextInt();
		for(int i =0; i < iter; i++){
			char[][] board = scan.nextCharMatrixFromString(scan.nextInt(), scan.nextInt());
			boolean cango[] = new boolean[26];
