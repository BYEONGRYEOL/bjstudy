...
public class boj_2406_안정적인네트워크 {
    ...
    static void solve() throws Exception {
        ...
        int[][] initUnion = new int[m][2];
        for (int i = 0; i < m; i++) {
            for (int index = 0; index < 2; index++) {
                initUnion[i][index] = scan.nextInt();
            }
        }
        ...
        for (int i = 1; i < n + 1; i++) {
            for (int j = 1; j < n + 1; j++) {
                if (i == j)
                    continue;
                State state = new State(0, initUnion);
                if (!state.hasRoute(i, j)) {
                    state.addRoute(i, j, adj[i][j]);
                    pq.add(state);
                }
            }
        }
        while (!pq.isEmpty()) {
            ...
            sb.append(state.getCost()).append('\n');
            break;
    ...
    static class State implements Cloneable {
        ...
        public void addRoute(int a, int b, int cost) {
            union(a, b);
        ...
        public State copyState(State state) {
            State newState = new State(state.cost, state.nodes.toArray(new int[0][0]));
            for (int i = 1; i < n + 1; i++) {
                newState.parent[i] = state.parent[i];
            }

            return newState;
        }

        public State(int cost, int[][] initUnion) {
            for (int i = 0; i < n + 1; i++) {
                parent[i] = i;
            }
            this.cost = cost;
            for (int i = 0; i < initUnion.length; i++) {
                union(initUnion[i][0], initUnion[i][1]);
            }
        }

        public State(int cost, int[] parent, List<int[]> nodes) {
            this.nodes = nodes;
            this.parent = parent;
            this.cost = cost;
        }
        ...
        public State clone() throws CloneNotSupportedException {
            ...
            newState.nodes = new ArrayList<>(this.nodes);
            newState.parent = Arrays.copyOf(this.parent, this.parent.length);
    ...
    static class PlaneNormal3D {
        ...
        @Override
        public String toString() {
            return String.format("Plane(Vector) : %f(x - %f) + %f(y - %f) + %f(z - %f) = 0", normalV.x, p.x,
                    normalV.y,
                    p.y, normalV.z, p.z);
        }
    }
    ...
    static long innerProduct(int[] point1, int[] point2, int[] point3) {
        return (long) (point2[0] - point1[0]) * (point2[1] - point1[1])
                + (long) (point3[0] - point1[0]) * (point3[1]
                        - point1[1]);
    }
