...
public class boj_1342_행운의문자열 {
	...
	static int luckyStringCount = 0;

	static void uniquePermutation(Map<Character, Integer> map, int remain, String result) {
		if (remain == 0) {
			luckyStringCount++;
		}
		for (Map.Entry<Character, Integer> entry : map.entrySet()) {
			if (result.isBlank() || result.charAt(result.length() - 1) != entry.getKey()) {
				char c = entry.getKey();
				int count = entry.getValue();
				if (count > 0) {
					map.put(c, count - 1);
					uniquePermutation(map, remain -1, result + c);
					map.put(c, count);
				}
	...
	static void solve() throws Exception {
		...
		uniquePermutation(map, input.length(), "");
		sb.append(luckyStringCount);
	}
