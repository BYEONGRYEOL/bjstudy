...
// public class boj_12906_새로운하노이탑 {
...
// char node = inputs[index].charAt(i);
// switch (node) {
// case 'A':
// A.push(node);
// counts[0]++;
...
// }
// }
// String answer = String.join(",", "A".repeat(counts[0]),
// "B".repeat(counts[1]), "C".repeat(counts[2]));
// Set<String> visited = new HashSet<>();

// Queue<Hanoi> q = new LinkedList<>();

// q.add(new Hanoi(initStr, 0));
// String[] standard = new String[] { "A", "B", "C" };
// while (!q.isEmpty()) {
// Hanoi hanoi = q.poll();
// // 방문검사
// if (visited.contains(hanoi)) {
// continue;
// }
// visited.add(initStr);
...
// String now = "";
// // 현재상태
// String[] state = hanoi.val.split(",", -1);
...
// for (int j = 0; j < 3; j++) {
// if (i != j) {
// if (state[i].contains(standard[j])) {
// success = false;
// }
// }
// }
...
// // 기둥에 꽂혀있으면
// if (state[i].length() > 0) {
// now = state[i].substring(state[i].length() - 1);
// // 다른 기둥에 넣을 궁리
// for (int j = 0; j < 3; j++) {
// if (i != j) {
// state[i] = state[i].substring(0, state[i].length() - 1);
// state[j] += now;
// // sb.append(state[i]).append(" -> ").append(state[j]).append('\n');
// String newState = String.join(",", state);
// if (!visited.contains(newState)) {
// // 다음 방문대기큐에 넣기
// q.add(new Hanoi(newState, hanoi.moveCount + 1));
// }
// state[i] += now;
// state[j] = state[j].substring(0, state[j].length() - 1);
// }
// }
// }
// }
...
// public Hanoi(String hanoi, int moveCount) {
// this.val = hanoi;
...
// return String.format("Ray : (%f,%f) + t(%f,%f) <for t>0>", p.x, p.y, v.x,
// v.y);
// }
// }
...
// @Override
// public String toString() {
// return String.format("Line : (%f,%f) + t(%f,%f) <any t>", p.x, p.y, v.x,
// v.y);
// }
// }
...
// @Override
// public String toString() {
// return String.format("LineSegment : (%f,%f,%f) -> (%f,%f,%f) ", p1.x, p1.y,
// p1.z, p2.x, p2.y, p2.z);
// }
// }
...
// @Override
// public String toString() {
// return String.format("LineSegment : (%f,%f) -> (%f,%f) ", p1.x, p1.y, p2.x,
// p2.y);
// }
// }
...
// @Override
// public String toString() {
// return String.format("Line : (%f, %f, %f) + t(%f, %f, %f", p.x, p.y, p.z,
// v.x, v.y, v.z);
// }
// }
...
// @Override
// public String toString() {
// return String.format("Plane(Vector) : %f(x - %f) + %f(y - %f) + %f(z - %f) =
// 0", normalV.x, p.x, normalV.y,
// p.y, normalV.z, p.z);
// }
// }
...
// this.vertices = vertices;
// Arrays.sort(this.vertices, Comparator.comparingDouble(p -> Math.atan2(p.y,
// p.x)));
// }
...
// int intersectionW = Math.max(0, Math.min(box1[2], box2[2]) -
// Math.max(box1[0], box2[0]));
// int intersectionH = Math.max(0, Math.min(box1[3], box2[3]) -
...
// return (long) (point2[0] - point1[0]) * (point2[1] - point1[1]) + (long)
// (point3[0] - point1[0]) * (point3[1]
// - point1[1]);
// }
...
// return (long) (b[0] - a[0]) * (long) (b[0] - a[0]) + (long) (b[1] - a[1]) *
// (long) (b[1] - a[1]);
// }
