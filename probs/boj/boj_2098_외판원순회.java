...
public class boj_2098_외판원순회 {
	...
	static int tsp(int pos, int mask) {
		// 모든 도시를 방문한 경우, 5개 도시를 예로 들면 32 - 1 -> 11111
		// 이 계산의 시작은 0번 도시를 방문하면서 시작했으므로, 현재 도시에서 0번 도시로 가는 비용을 반환해야한다.
		if (mask == (1 << n) - 1) {
			return dist[pos][0] != 0 ? dist[pos][0] : Integer.MAX_VALUE / 2; // 여기 -1은 의미가 없음
		}

		if (dp[pos][mask] > -1) {
			return dp[pos][mask];
		}
		// 방문할수 있는 모든 도시를 방문하면서,
		// 현재 도시에서 해당 도시를 방문한 후 나머지 경로는 재귀로 계산
		for (int i = 0; i < n; i++) {
			// 경로가 없는 경우와 이미 방문한 경우를 모두 여기서 검사함
			if ((mask & (1 << i)) == 0 && dist[pos][i] != 0) {
				// 따라서 아래 계산이 한번이라도 수행된 경우 경로가 적어도 한개는 존재함을 의미함
				// mask | (1<<i) 의 의미는 현재까지의 방문한 도시 mask와
				// dist[pos][i] 를 계산하면서 처리되는 이미 방문한 도시 i를 모두 1로 마스킹하여 남은 0에 해당하는 미방문 도시만 방문토록
				if (dp[pos][mask] == -1) {
					dp[pos][mask] = dist[pos][i] + tsp(i, mask | (1 << i));
				} else {
					dp[pos][mask] = Math.min(dp[pos][mask], dist[pos][i] + tsp(i, mask | (1 << i)));
				}
			}
		}
		// 여기까지 왔을 때, 만약 min값이 초기화되지 못했다면? 0번으로 돌아가는 경로가 없는 것임
		if (dp[pos][mask] == -1) {
			// 방문해봤으나(-1이 아님) 경로가 없어 최댓값(2e8 <- 2e9는 정수 오버플로우 가능성 있음)
			dp[pos][mask] = Integer.MAX_VALUE / 2;
		}
		return dp[pos][mask];
	}
