...
public class boj_12100_2048_EASY {
	...
	static void solve() throws Exception {
		...
		for (int i = 0; i < 4; i++) { // 0: 위, 1: 아래, 2: 왼쪽, 3: 오른쪽
			maxTile =  Math.max(move(board, 0, i), maxTile);
		}
		sb.append(maxTile).append("\n");
	}

	static int move(int[][] board,int depth, int direction) {
		int n = board.length;
		...
		if (direction == 0) { // 위로 이동
			for (int j = 0; j < n; j++) {
				...
				for (int i = 0; i < n; i++) {
					if (board[i][j] != 0) {
						...
						} else if (newBoard[idx][j] == board[i][j] && !merged[idx][j]) {
							newBoard[idx][j] *= 2;
							merged[idx][j] = true;
							idx++;
						} else {
			...
			for(int dir = 0; dir < 4; dir++){
				max = Math.max(max,move(newBoard, depth + 1, dir));
			} 
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < n; j++) {
					if (board[i][j] != 0) {
						...
						} else if (newBoard[i][idx] == board[i][j] && !merged[i][idx]) {
							newBoard[i][idx] *= 2;
							merged[i][idx] = true;
							idx++;
						} else {
