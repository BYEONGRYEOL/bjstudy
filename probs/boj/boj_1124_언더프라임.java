...
public class boj_1124_언더프라임 {
	...
	static void solve() throws Exception {
		int l = scan.nextInt();
		int r = scan.nextInt();
		Prime_Eratosthenes prime = new Prime_Eratosthenes(r);
		int cur = l;
		int underPrimeCount = 0;
		while(cur <= r){
			int num = cur;
			int primeFactorCount = 0;
			for(int p : prime.primeArray){
				if(p*p > num) {
					if(num > 1){
						num = 1;
						primeFactorCount++;
			...
			if(num > 1) primeFactorCount++;
			if(prime.isPrime[primeFactorCount]){
				underPrimeCount++;
			}
			cur++;
		}
		sb.append(underPrimeCount);
	}
