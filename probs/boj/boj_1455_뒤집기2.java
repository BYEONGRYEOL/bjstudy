...
public class boj_1455_뒤집기2 {
	...
	static void solve() throws Exception {
		...
		int[][] board = (int[][])Arrays.stream(scan.nextStringArray(n)).map(d -> Arrays.stream(d.split("")).mapToInt(Integer::parseInt).toArray()).toArray();
		int count = 0;
		while (true) {
			int[] backCoinIdx = findBackCoin(board);
			if (backCoinIdx[0] == -1)
				break;
			flip(board, backCoinIdx);
			count++;
		}
		sb.append(count);
	}
