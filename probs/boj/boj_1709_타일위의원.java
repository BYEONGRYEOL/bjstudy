...
public class boj_1709_타일위의원 {
	...
	static void solve() throws Exception {
		// 사분원의 둘레에 있는 두 점을 기준으로 각 x좌표마다 최소한 한개씩의 원과 단위정사각형의 교점이 발생한다.
		// 해당하는 교점의 개수는 반지름의 길이와 같고,
		// 원을 따라가면서 정확히 단위정사각형의 꼭지점과 원이 만나는 경우가 아니라면 교점이 한개씩 더 발생한다.
		//
		int N = scan.nextInt();
		int r = N/2;
		long count = r;
		for (int i = 1; i <= r; i++) {
			long ySquare = (long)r *r - (long)i *i;
			long sqrt = (long)Math.sqrt(ySquare);
			if(sqrt * sqrt != ySquare){
				count++;
			}
		}
