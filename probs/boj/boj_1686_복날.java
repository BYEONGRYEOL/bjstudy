...
public class boj_1686_복날 {

	static void solve() throws Exception {
		int START = 0;
		int TARGET = 1;
		double v = scan.nextDouble();
		double time = scan.nextDouble() * 60;
		List<double[]> bunkers = new ArrayList<>();
		double[] start = scan.nextDoubleArray(2);
		bunkers.add(start);
		double[] target = scan.nextDoubleArray(2);
		bunkers.add(target);
		String input = "";
		while((input = scan.nextLine()) !=null){
		    double[] bunker = new double[2];
			String[] split = input.split(" ");
			bunker[0] = Double.parseDouble(split[0]);
			bunker[1] = Double.parseDouble(split[1]);
			bunkers.add(bunker);
		}
		Map<Integer, List<Integer>> depths = new HashMap<>();
		int[] depth = new int[bunkers.size()];
		depths.put(0, new ArrayList<>());
		depths.get(0).add(TARGET);
		int curDepth = 1;
		while(true){
			for (int i = 0; i < bunkers.size(); i++) {
				// System.out.println("curDepth = " + curDepth);
				List<Integer> depthList = depths.get(curDepth-1);
				if(depth[i] == 0){
					for (Integer bunkerIdx : depthList) {
						// System.out.println("distance(tower[i], spot) : " + distance(tower[i], spot));
						if (depth[i] == 0 && distance(bunkers.get(i), bunkers.get(bunkerIdx)) <= time*v) {
							// System.out.printf("curDepth : %d , (%d, %d) add because (%d, %d) distance  \n", curDepth, tower[i][0], tower[i][1], spot[0], spot[1]);
							depths.putIfAbsent(curDepth, new ArrayList<>());
							depths.get(curDepth).add(i);
							depth[i] = curDepth;
						}
					}
				}
			}
			if(depths.get(curDepth) == null){
				break;
			}
			curDepth++;
		}
		for (Map.Entry<Integer, List<Integer>> entry : depths.entrySet()){
			if(entry.getKey() > 0){
				if(entry.getValue().contains(START)){
					sb.append(String.format("Yes, visiting %d other holes.\n", entry.getKey()-1));
					return;
				}
			}
		}
