...
public class boj_2536_버스갈아타기 {

    static void solve() throws Exception {
        int m = scan.nextInt();
        int n = scan.nextInt();
        int bus = scan.nextInt();
        int[][] buses = new int[bus][4];
        for (int i = 0; i < bus; i++) {
            scan.nextInt();
            buses[i] = scan.nextIntArray(4);
        }
        List<Integer>[] graph = new List[bus];
        for (int i = 0; i < bus; i++) {
            graph[i] = new ArrayList<>();
        }
        for (int i = 0; i < bus; i++) {
            for (int j = i + 1; j < bus; j++) {
                if (isCrossing(buses[i], buses[j])) {
                    graph[i].add(j);
                    graph[j].add(i);
                }
            }
        }
        int minDist[] = new int[bus];

    }

    static boolean isCrossing(int[] a, int[] b) {
        boolean aVertical = a[0] == a[2];
        boolean bVertical = b[0] == b[2];
        if (aVertical == bVertical) {
            ...
            if (Math.min(a[idx1], a[idx2]) > Math.max(b[idx1], b[idx2])
                    || Math.min(b[idx1], b[idx2]) > Math.max(a[idx1], a[idx2])) {
                return false;
            } else {
                return true;
            }
        } else {
            if (bVertical) { // b 수직, a 수평
                int[] temp = Arrays.copyOf(a, a.length);
                a = Arrays.copyOf(b, b.length);
                b = temp;
            }
            // a 수직, b 수평
            boolean verticalOK = a[0] <= Math.max(b[0], b[2]) && a[0] >= Math.min(b[0], b[2]);
            boolean horizontalOK = b[1] <= Math.max(a[1], a[3]) && b[1] >= Math.min(a[1], a[3]);
