...
public class boj_16973_직사각형탈출 {
    ...
    static void solve() throws Exception {
        ...
        int tx = scan.nextInt() - 1;
        int ty = scan.nextInt() - 1;
        ...
        while (!queue.isEmpty()) {
            ...
            // sb.append(cx).append(" ").append(cy).append(" ").append(curDepth).append("
            // ").append("\n");
            ...
            for (int k = 0; k < 4; k++) {
                ...
                int newx = cx + dirx[k];
                int newy = cy + diry[k];
                if (newx + h > N || newy + w > M || newx < 0 || newy < 0 || visited[newx][newy]) {
                    continue;
                }
                // 0 1 2 3 우 하 좌 상
                if (k == 0) {
                    for (int i = 0; i < h; i++) {
                        sum += board[cx + i][newy + w - 1];
                    }
                    // newy + w 의 newx 부터 newx + h 까지 합
                    if (sum > 0) {
                        continue;
                    }
                }
                if (k == 1) {
                    for (int i = 0; i < w; i++) {
                        sum += board[newx + h - 1][cy + i];
                    }
                    // newx + h 의 newy 부터 newy + w 까지 합
                    if (sum > 0) {
                        continue;
                    }
                }
                ...
                queue.add(new int[] { newx, newy, curDepth + 1 });
            }
        }
        sb.append(-1);
    }
