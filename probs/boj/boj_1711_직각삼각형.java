...
public class boj_1711_직각삼각형 {
	static void solve() throws Exception {
		int n = scan.nextInt();
		int[][] points = scan.nextIntMatrix(n,2);
		long rightTriangleCount = 0;
		for (int i = 0; i < n - 2; i++) {
			for (int j = i+1; j < n-1; j++) {
				for (int k = j+1; k < n; k++) {
					long d1 = distanceSquare(points[i], points[j]);
					long d2 = distanceSquare(points[j], points[k]);
					long d3 = distanceSquare(points[k], points[i]);
