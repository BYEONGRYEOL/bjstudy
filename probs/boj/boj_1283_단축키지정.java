...
public class boj_1283_단축키지정 {
	...
	static void solve() throws Exception {
		...
		boolean[] keybindings = new boolean[26];
		for(int i =0; i < iter; i++){
			String[] command = scan.nextLine().split(" ");
			boolean success = false;
			for (int j = 0; j < command.length; j++) {
				if(!keybindings[aTo0(command[j].charAt(0))]){
					keybindings[aTo0(command[j].charAt(0))] = true;
					command[j] = "[" + command[j].charAt(0) + "]" + command[j].substring(1);
					success = true;
					break;
				}
			}
			if(!success){
				for (int j = 0; j < command.length; j++) {
					if(success) break;
					for (int k = 0; k < command[j].length(); k++) {
						if(!keybindings[aTo0(command[j].charAt(k))]){
							keybindings[aTo0(command[j].charAt(k))] = true;
							command[j] = command[j].substring(0, k) + "[" + command[j].charAt(k) + "]" + command[j].substring(k+1);
							success = true;
							break;
						}
					}
