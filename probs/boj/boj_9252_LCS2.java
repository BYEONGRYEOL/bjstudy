...
public class boj_9252_LCS2 {
	...
	static void solve() throws Exception {
		l1 = scan.nextLine().toCharArray();
		l2 = scan.nextLine().toCharArray();
		int len1 = l1.length;
		int len2 = l2.length;
		dp = new int[len1 + 1][len2 + 1];
		// dp[i][j] 는 a[0~i] b[0~j] 까지의 문자열을 모두 비교해봤을때 가장
		// 가장 긴 LCS의 길이를 나타낸다.
		...
		sb.append(getLCSString(len1,len2, ""));

	}

	static String getLCSString(int n, int m, String lcs) {
		if (dp[n][m] == 0)
			return lcs;
		if (l1[n-1] == l2[m-1]){
			lcs = l1[n-1] + lcs;
			return getLCSString(n-1,m-1,lcs);
		} else{
			if(dp[n-1][m] > dp[n][m-1]){
				return getLCSString(n-1,m,lcs);
			} else{
				return getLCSString(n,m-1,lcs);
			}
		}
	}
