...
public class boj_1700_멀티탭스케쥴링 {
	...
	static void solve() throws Exception {
		n = scan.nextInt();
		k = scan.nextInt();
		order = scan.nextIntArray(k);
		int[] code = new int[n];
		// dp?
		// 2번을 뽑았을 때 이후 최소, 3번을 뽑았을 때 이후 최소
		sb.append(schedule(code, 0, 0));

	}

	static int schedule(int[] code, int idx, int count) {
		...
		for (int i = 0; i < code.length; i++) {
			if (code[i] == now) {
				return schedule(code, idx + 1, count);
			}
			if (code[i] == 0) {
				code[i] = now;
				return schedule(code, idx + 1, count);
		...
		code[popIndex] = now;
		return schedule(code, idx + 1, count + 1);
	}
