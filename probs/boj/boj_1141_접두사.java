...
public class boj_1141_접두사 {
	...
	static void solve() throws Exception {
		int n = scan.nextInt();
		String[] inputs = scan.nextStringArray(n);
		Arrays.sort(inputs);
		// 길이가 짧은순으로 정렬되며, 접두사가 될 수 있는 가능성은 인접한 문자열에 한정되므로,
		// 누군가의 접두사가 될 수 있어 2개 문자열 중 한개를 포기해야할 때, 무조건 정렬순서가 앞쪽인 문자열을 포기하도록
		int discardCount = 0;
		String prev = inputs[0];
		for (int i = 1; i < n; i++) {
			if(inputs[i].charAt(0) == prev.charAt(0)) {
				if (inputs[i].startsWith(prev)) {
					discardCount++;
				}
			}
			prev = inputs[i];
		}
		sb.append(n - discardCount);
