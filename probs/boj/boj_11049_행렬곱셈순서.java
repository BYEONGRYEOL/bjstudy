...
public class boj_11049_행렬곱셈순서 {
	static void solve() throws Exception {
		// 행렬이 2개일때의 최소 계산횟수는 강제로 m1*m2 의 값을 가짐,
		// 행렬이 3개일 때의 최소 계산횟수는 아래와 같음
		// (m1*m2)*m3
		// m1*(m2*m3)
		// 이는 모든 연달아있는 2개 행렬에 대하여 계산횟수를 알고 있다면, 행렬개수 N-1 만큼만 비교하면 최적을 찾을 수 있다.

		int n =scan.nextInt();
		int[][] matrix = scan.nextIntMatrix(n,2);
		long[][] dp = new long[n][n];
		for (int i = 0; i < n - 1; i++) {
			dp[i][i+1] = (long)matrix[i][0] * matrix[i][1] * matrix[i+1][1];
			// System.out.println(String.format("dp[%d][%d] : %d", i, i+1, dp[i][i+1]));
		}
		for (int i = 2; i < n; i++) {
			for (int j = 0; j < n-i; j++) {
				dp[j][j+i] = Long.MAX_VALUE;
				for (int k = j; k < j+i; k++) {
					dp[j][j+i] = Math.min(dp[j][j+i], dp[j][k]+dp[k+1][j+i] + (long)matrix[j][0] * matrix[k][1] * matrix[j+i][1]);
