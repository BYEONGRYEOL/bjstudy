...
public class boj_4716_풍선 {
    ...
    static void solve() throws Exception {
        while (true) {
            ...
            if (t == 0)
                break;
            int[] balloons = scan.nextIntArray(2);
            PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> {
                return Math.abs(b[0] - b[1]) - Math.abs(a[0] - a[1]);
            });
            for (int i = 0; i < t; i++) {
                int count = scan.nextInt();
                int aDist = scan.nextInt();
                int bDist = scan.nextInt();
                pq.add(new int[] { aDist, bDist, count });
            }
            long dist = 0;
            while (!pq.isEmpty()) {
                int[] now = pq.poll();
                ...
                if (now[2] > balloons[idx]) {
                    dist += balloons[idx] * now[idx];
                    dist += (now[2] - balloons[idx]) * now[1 - idx];
                    balloons[1 - idx] -= (now[2] - balloons[idx]);
                    balloons[idx] = 0;
                } else {
                    dist += now[2] * now[idx];
                    balloons[idx] -= now[2];
