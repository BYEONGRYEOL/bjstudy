...
public class boj_1166_선물 {
	...
	static void solve() throws Exception {
		...
		int[] lengths = scan.nextIntArray(3);
		// 최댓값은 반드시 L,W,H 중 적어도 1개의 값을 모두 사용한 형태가 될 것,
		// 각각의 모든 경우를 계산하여 최댓값을 산출한다.
		double maxLen = 0;
		for (int i = 0; i < 3; i++) {
			double l = 0d;
			double r = Math.min(lengths[0], Math.min(lengths[1], lengths[2]));
			double mid = (l + r) / 2;
			while (l < r) {
				mid = (l + r) / 2;
				if (boxCount(lengths, mid) < target) {
					if (mid == r)
						break;
					r = mid;
				} else {
