...
public class boj_11664_선분과점 {

	static void solve() throws Exception {
		Point3D A = new Point3D(scan.nextDoubleArray(3));
		Point3D B = new Point3D(scan.nextDoubleArray(3));
		Point3D P = new Point3D(scan.nextDoubleArray(3));
		Vector3D AB = new Vector3D(B,A); // 벡터 AB
		Vector3D AP = new Vector3D(P,A);

		// t 계산
		double t = AP.dot(AB) / AB.magnitudeSquared();

		if (t < 0) {
			sb.append(P.distance(A));
		} else if (t > 1) {
			sb.append(P.distance(B));
		} else {
			// P는 선분의 내부 점과 가장 가까움
			Point3D projection = A.add(AB.multiply(t));
			sb.append(P.distance(projection)).append('\n');
		}
	}
