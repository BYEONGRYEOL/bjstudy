...
public class boj_1105_팔 {
	static void solve() throws Exception {
		...
		String max = numbers[1];
		int digitDiff = max.length() - min.length();
		if(digitDiff > 0){
			min = "0".repeat(digitDiff) + min;
		}
		int idx =0;
		int required8Count = 0;
		boolean isContinuous8 = true;
		while(idx < max.length() && isContinuous8){
			char maxC = max.charAt(idx);
			char minC = min.charAt(idx);
			if(minC == maxC){
				if(minC == '8')
					required8Count++;
			} else{
