...
public class boj_1303_전쟁_전투 {
	...
	static void solve() throws Exception {
		int n = scan.nextInt();
		int m = scan.nextInt();
		board = scan.nextCharMatrixFromString(m, n);
		visited = new boolean[m][n];
		Map<Character, Long> strength = new HashMap<Character, Long>();
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				if (!visited[i][j]) {
					strength.merge(board[i][j], (long)Math.pow(bfs(i, j), 2), Long::sum);
				}
			}
		}
		sb.append(strength.get('W')).append(' ').append(strength.get('B')).append('\n');
	}
