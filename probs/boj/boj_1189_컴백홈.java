...
public class boj_1189_컴백홈 {
	...
	static void solve() throws Exception {
		targetR = scan.nextInt() - 1;
		targetC = scan.nextInt() - 1;
		targetDistance = scan.nextInt();
		visited = new boolean[targetR + 1][targetC + 1];
		String[] inputs = scan.nextStringArray(targetR + 1);
		String temp = "";
		...
		for (int i = 0; i < targetR + 1; i++) {
			for (int j = 0; j < targetC + 1; j++) {
				if (inputs[i].charAt(j) == 'T') {
					visited[i][j] = true;
				}
			}
		}
	...
	static void dfs(int r, int c, int d) {
		if (r == targetR && c == targetC && d == targetDistance) {
			routeCount++;
			return;
		}
		if (d > targetDistance) {
		...
		for (int dir = 0; dir < 4; dir++) {
			int newR = r + dirR[dir];
			int newC = c + dirC[dir];
			if (newR < 0 || newR > targetR || newC > targetC || newC < 0 || visited[newR][newC]) {
				continue;
			}
