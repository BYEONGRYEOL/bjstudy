...
public class boj_1004_어린왕자 {
	...
	static void solve() throws Exception {
		...
		for (int i = 0; i < iter; i++) {
			Point2D s = new Point2D(scan.nextInt(), scan.nextInt());
			Point2D e = new Point2D(scan.nextInt(), scan.nextInt());
			int count = 0;
			int n = scan.nextInt();
			for (int j = 0; j < n; j++) {
				Point2D center = new Point2D(scan.nextInt(), scan.nextInt());
				int r = scan.nextInt();
				Circle2D circle2D = new Circle2D(center, r);
				count += circle2D.isIn(s) ^ circle2D.isIn(e) ? 1 : 0;
			}
