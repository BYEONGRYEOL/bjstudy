...
public class boj_1773_폭죽쇼 {
	static void solve() throws Exception {
		...
		boolean[] firecracker = new boolean[endTime+1];
		for (int i = 0; i < n; i++) {
			int cycle = scan.nextInt();
			for (int j = cycle; j <= endTime; j+=cycle) {
				firecracker[j] = true;
			}
		}
		int firecrackerTime = 0;
		for (int i = 1; i < endTime+1; i++) {
			if(firecracker[i]) firecrackerTime++;
		}
		sb.append(firecrackerTime);
	}
