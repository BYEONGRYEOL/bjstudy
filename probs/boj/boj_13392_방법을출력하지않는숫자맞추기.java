...
public class boj_13392_방법을출력하지않는숫자맞추기 {
    static int[][] dp;
    ...
    static void solve() throws Exception {
        ...
        for (int i = 0; i < n; i++) {
            target[i] = ((after.charAt(i) - before.charAt(i)) + 10) % 10;
        }
        ...
        sb.append(minMove(target, 0, 0));
    }

    static int minMove(int[] target, int idx, int left) {
        ...
        // 좌회전이 +1
        int cur = (target[idx] - left + 10) % 10;
        int leftMove = cur;
        int rightMove = (10 - cur) % 10;
        dp[idx][left] = Math.min(minMove(target, idx + 1, left) + rightMove,
                minMove(target, idx + 1, (left + leftMove) % 10) + leftMove);
