...
public class boj_32953_회상 {
	static void solve() throws Exception {
		...
		Map<Integer, Integer> stuNumMap = new HashMap<Integer, Integer>();
		for(int i =0; i < iter; i++){
		    int n = scan.nextInt();
			for (int j = 0; j < n; j++) {
				stuNumMap.merge(scan.nextInt(), 1, Integer::sum);
			}
		}
		int count = 0;
		for (Map.Entry<Integer, Integer> entry : stuNumMap.entrySet()) {
