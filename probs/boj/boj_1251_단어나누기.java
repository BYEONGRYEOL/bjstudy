	...
	static void solve() throws Exception {
		...
		for (int i = 1; i < input.length() - 1; i++) {
			for (int j = i + 1; j < input.length(); j++) {
				...
				String result = tempsb.append(a).reverse().toString();
				tempsb.setLength(0);
				result += tempsb.append(b).reverse().toString();
				tempsb.setLength(0);
				result += tempsb.append(c).reverse().toString();
				tempsb.setLength(0);
				if (answer.isEmpty()) {
					answer = result;
				} else if (answer.compareTo(result) >= 0) {
					answer = result;
				}
			}
		}
