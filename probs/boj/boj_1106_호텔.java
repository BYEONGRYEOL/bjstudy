...
public class boj_1106_호텔 {
	...
	static void solve() throws Exception {
		int C = scan.nextInt(); // 고객 수
		int N = scan.nextInt(); // 호텔 수
		int[][] costAndCustomer = new int[N][2];
		for (int i = 0; i < N; i++) {
			costAndCustomer[i][0] = scan.nextInt(); // 비용
			costAndCustomer[i][1] = scan.nextInt(); // 고객 수
		}
		int[] dp = new int[C+101];
		Arrays.fill(dp, Integer.MAX_VALUE);
		dp[0] = 0; // 고객 수가 0일 때 비용은 0
		for(int i = 1; i < C+100; i++){
			for(int j = 0 ; j< N; j++){
				int cost = costAndCustomer[j][0];
				int customer = costAndCustomer[j][1];
				if(i >= customer && dp[i - customer] != Integer.MAX_VALUE){
