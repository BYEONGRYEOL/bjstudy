...
public class boj_1072_게임 {
	...
	static void solve() throws Exception {
		...
		long win = scan.nextInt();
		long winRate = win * 100 / total;
		if (winRate >= 99) {
			sb.append(-1);
			return;
		}
		long l = 1;
		long r = 1;
		while (!check(win, total, r, winRate)) {
			r *= 2;
		}
		while (l < r) {
			long mid = (l + r) / 2;
			if (check(win, total, mid, winRate)) {
				r = mid;
			} else {
				l = mid + 1;
			}
	...
	static boolean check(long win, long total, long d, long winRate) {
		return (win + d) * 100 / (total + d) > winRate;
	}
